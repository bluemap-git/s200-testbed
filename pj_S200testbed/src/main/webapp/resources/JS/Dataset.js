				...
				tableData.each(function() {
					for (var d = 0; d < id.length; d++) {
						...
						if ($(this).text() == testput) {
							$(this).parent().find('td:first-child :checkbox')
									.prop("checked", true);
							$(this).parent().find('td:eq(2)')
									.addClass('active');
						}
					}
				});
...
function setValidationResult(resultObject) {
	for (var i = 0; i < resultObject.length; i++) {
		...
		var record = "<tr class=\"checktable\"><td>" + id + "</td>" + "<td>"
				+ message + "</td></tr>";
...
$('.on_off').on(
		...
		function() {
			...
			document.getElementsByClassName("on_off")[onfoffindex].classList
					.toggle("menutomato");
			...
			for (var i = 0; i < len; i++) {
				if (i != onfoffindex) {
					...
					document.getElementsByClassName("on_off")[i].classList
							.remove("menutomato");
				}
			}
					...
					.addListener(
							...
							(function(marker, i) {
								return function() {
									...
									var data_ds_dix = document
											.getElementsByClassName("data_ds_dix");
									for (var s = 0; s < data_ds_dix.length; s++) {
										if (data_ds_dix[s].innerText == putmarkeridx) {
											data_ds_dix[s].parentElement.children[5].childNodes[0].classList
													.add("menutomato");
										} else {
											data_ds_dix[s].parentElement.children[5].childNodes[0].classList
													.remove("menutomato");
